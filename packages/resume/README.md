...
Seven CV focuses × two languages = fourteen static pages + fourteen PDFs.
...
- Routes: `src/pages/resume.astro` (Universal EN) + `src/pages/resume/[role]/[lang].astro` (`getStaticPaths` for the other nine).
...
```bash
pnpm build            # astro check + astro build + all 10 PDFs into dist/resume/pdfs/
