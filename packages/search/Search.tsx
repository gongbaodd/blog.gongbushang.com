  ...
  return (
    <CustomMantineProvider>
      ...
      >
        ...
        {!isLoading && (
          <Text span c="dimmed" size="xs" ml="1em">
            <Kbd>⌘</Kbd>+<Kbd>K</Kbd>
          </Text>
        )}
      </Button>
...
function SpotlightModal({ onSpotlightClose }: { onSpotlightClose: () => void }) {
  ...
  const actions: ReactNode[] = posts.map((post) => (
    <Spotlight.Action
      key={post.id}
      px={0}
      onClick={() => (location.href = `/${post.category.label}/${post.id}`)}
    ...
    >
      <Group justify="space-between" align="flex-start" p={"xs"}>
        <Box>
          ...
          {"queryTerms" in post &&
            (post as SearchResult)?.queryTerms.map((q) => (
  ...
  return (
    <Spotlight.Root
      ...
      style={query ? { height: "auto" } : undefined}
    >
