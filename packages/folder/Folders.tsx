    ...
    <CustomMantineProvider>
      <Container fluid style={{ marginInline: "initial" }} p={0} mih={"50vh"}>
        <Flex wrap={"wrap"} className={classes.grid}>
          {Object.keys(counts)
            .filter((year) => year !== FILTER_ENTRY.ALL)
            .reverse()
            .map((year) => {
                  ...
                  {openedYear === year && (
                    <Center style={{ position: "absolute" }}>
                      <Anchor href={"/year/" + year}>
                        <Button radius={"md"} size="xl" variant="gradient">
                          View {counts[year].toString()} posts
                        </Button>
                      </Anchor>
                    </Center>
                  )}
...
function FolderCover({ year, heatmap }: { year: string; heatmap: IHeatResult["heatData"] }) {
  return (
    <Stack gap={0} className={classes.cover} justify="center" align="center">
      <Text c={"white"}>{year}</Text>
      <Group
        display={"block"}
        style={{ overflow: "hidden", borderRadius: "var(--mantine-radius-sm)" }}
      >
        {[`${year}-01-01`, `${year}-07-01`].map((day) => (
          <Heat key={day} data={heatmap} startDate={day} />
        ))}
      </Group>
    </Stack>
  );
