...
const STATIC_ENTRIES = [FILTER_ENTRY.ALL, FILTER_ENTRY.TAG, FILTER_ENTRY.SERIES];
...
const getMemorizedCategoryFilterEntries = memoize(getCategoryFilterEntries, {
  getCacheKey: (posts: T_POST[]) => posts.length.toString(),
});
...
export const getAllFilterEntries = async () => {
  ...
  return [...entries, ...categoryEntries];
};

let categoryPostMap = new Map<string, Set<T_POST>>();
const _initCategoryPostMap = memoize(
  (posts: T_POST[]) => {
...
const _initTagPostMap = memoize(
  (posts: T_POST[]) => {
    tagPostMap = createPostMap(posts, (p) =>
...
const _initCityPostMap = memoize(
  (posts: T_POST[]) => {
...
const _initSeriesPostMap = memoize(
  (posts: T_POST[]) => {
    seriesPostMap = createPostMap(posts, (p) => (p.data.series?.slug ? [p.data.series.slug] : []));
...
export function page(
  filterFn:
    | typeof getFilterByCityPage
    | typeof getFilterByCategoryPage
    | typeof getFilterBySeriesPage
    | typeof getFilterByTagPage
    | typeof getFilterByYearPage,
) {
  return async () => {
...
const _initYearPostMap = memoize(
  (posts: T_POST[]) => {
...
const _initYearMonthPostMap = memoize(
  (posts: T_POST[]) => {
    monthPostMap = createPostMap(posts, (p) => [dayjs(date(p)).format("YYYY-MM")]);
    return monthPostMap;
  },
  { getCacheKey: (posts) => Array.from(posts).length },
);
