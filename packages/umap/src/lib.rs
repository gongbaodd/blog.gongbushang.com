...
pub use rag_umap::Umap2dConfig;
use rag_umap::convert_to_2d_with_config;
...
pub fn umap_2d(
    ...
    let result = convert_to_2d_with_config(embeddings.to_vec(), config)
...
#[cfg(test)]
mod tests {
    use super::{umap_2d, Umap2dConfig};

    fn test_config() -> Umap2dConfig {
        Umap2dConfig {
            n_neighbors: 15,
            min_dist: 0.1,
            spread: 1.0,
    ...
    fn rejects_empty_input() {
        let error = umap_2d(&[], test_config()).unwrap_err();
        assert!(error.contains("empty"));
    }

    #[test]
    fn rejects_single_row() {
        let error = umap_2d(&[vec![1.0, 2.0, 3.0]], test_config()).unwrap_err();
        assert!(error.contains("at least 2"));
    }

    #[test]
    fn rejects_ragged_rows() {
        let error = umap_2d(&[vec![1.0, 2.0], vec![3.0]], test_config()).unwrap_err();
        assert!(error.contains("row 1"));
    }

    #[test]
    fn returns_four_2d_coordinates_for_synthetic_matrix() {
        let coordinates = umap_2d(&synthetic_matrix(), test_config()).expect("valid matrix");
