...
describe.runIf(process.env.DEPTH_TRACE_TEST === "1")("depth trace pipeline (integration)", () => {
  test.each(FIXTURES)(
    ...
    async (fixture) => {
      ...
      const depthSvg = await traceBufferToSvg(await sharpSobel(await prepareWithDepth(png)));

      assertValidSvg(baselineSvg, `${stem} baseline`);
      assertValidSvg(depthSvg, `${stem} depth`);

      await expect(baselineSvg).toMatchFileSnapshot(`./__snapshots__/${stem}-baseline.svg`);
      await expect(depthSvg).toMatchFileSnapshot(`./__snapshots__/${stem}-depth.svg`);
    },
    120_000,
  );
});
