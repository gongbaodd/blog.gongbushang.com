...
import { badgeVariants } from "../shadcn/components/ui/badge";
...
const DateFilter: FC<{ date: Date; filter: string }> = ({ date, filter }) => {
  ...
  return (
    <>
      <a key={monthLink} href={monthLink}>
        {monthName}
      </a>
      <a key={dayLink} href={dayLink}>
        {day}
      </a>
      ,
      <a key={yearLink} href={yearLink}>
        {year}
      </a>
    </>
  );
  ...
  return (
    <Card>
      <CardHeader className="relative">
        ...
        <a
          className={cn(badgeVariants({ variant: "secondary" }), "absolute top-0 right-1")}
          href={`/${category}`}
        >
          {category}
        </a>
      ...
      <CardFooter className="flex-row-reverse">
        <a className={buttonVariants({ variant: "outline" })} href={link}>
          Read More
        </a>
      </CardFooter>
    </Card>
